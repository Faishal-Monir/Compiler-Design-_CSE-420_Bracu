...
vector<decl_item> pending_decls;



vector<pair<string, string>> pending_params;
vector<pair<string, string>> params_for_next_scope;
bool insert_params_on_scope_enter = false;
...
func_definition : type_specifier ID LPAREN parameter_list RPAREN
		{
			...
			// insert function into the current (global) scope before entering function body scope
			vector<pair<string, string>> params = pending_params;
			symbol_info *func_symbol = new symbol_info($2->getname(), "ID");
			func_symbol->set_as_function($1->getname(), params);
			if (st != NULL && !st->insert(func_symbol))
			{
				delete func_symbol;
				yyerror((char *)"Duplicate function in current scope");
				YYABORT;
			}

			params_for_next_scope = params;
			insert_params_on_scope_enter = true;
		...
		| type_specifier ID LPAREN RPAREN
		{
			...
			insert_params_on_scope_enter = false;
			params_for_next_scope.clear();
...
parameter_list : parameter_list COMMA type_specifier ID
		...
		| parameter_list COMMA type_specifier
		{
			...
			pending_params.push_back({$3->getname(), ""});
		}
 		| type_specifier ID
 		{
		...
		| type_specifier
		{
			...
			pending_params.clear();
			pending_params.push_back({$1->getname(), ""});
		}
 		;
...
compound_statement : LCURL
			{
				...
				if (st != NULL && insert_params_on_scope_enter)
				{
					for (auto &p : params_for_next_scope)
					{
						if (p.second.empty())
							continue;

						symbol_info *param_symbol = new symbol_info(p.second, "ID");
						param_symbol->set_category("Variable");
						param_symbol->set_data_type(p.first);

						if (!st->insert(param_symbol))
						{
							delete param_symbol;
							yyerror((char *)"Duplicate parameter in current scope");
							YYABORT;
						}
					}
...
var_declaration : type_specifier declaration_list SEMICOLON
		 {
			...
			if (st != NULL)
			{
				for (auto &item : pending_decls)
				{
					symbol_info *sym = new symbol_info(item.name, "ID");
					if (item.is_array)
					{
						sym->set_category("Array");
						sym->set_data_type(current_decl_type);
						sym->set_array_size(item.array_size);
					}
					else
					{
						sym->set_category("Variable");
						sym->set_data_type(current_decl_type);
					}
					...
					{
						delete sym;
						yyerror((char *)"Duplicate declaration in current scope");
						YYABORT;
					}
				}
			}
...
declaration_list : declaration_list COMMA ID
		  {
			...
			pending_decls.push_back({$3->getname(), false, -1});
 		  ...
 		  | declaration_list COMMA ID LTHIRD CONST_INT RTHIRD //array after some declaration
 		  {
			...
			pending_decls.push_back({$3->getname(), true, stoi($5->getname())});
 		  ...
 		  |ID
 		  {
			...
			pending_decls.clear();
			pending_decls.push_back({$1->getname(), false, -1});
 		  ...
 		  | ID LTHIRD CONST_INT RTHIRD //array
 		  {
			...
			pending_decls.clear();
			pending_decls.push_back({$1->getname(), true, stoi($3->getname())});
			$$ = new symbol_info($1->getname()+"["+$3->getname()+"]", "declaration_list");
